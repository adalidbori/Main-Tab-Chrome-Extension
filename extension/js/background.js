...
// Send a message to the active tab
  chrome.tabs.query({currentWindow: true, url: chrome.extension.getURL('maintab.html')}, function(tabs) {
    //makes tabbie active since it's already open
    if(tabs[0] !== undefined){
      chrome.tabs.update(tabs[0].id, {active: true});
    }
    else{
      chrome.tabs.create({ url: chrome.extension.getURL('maintab.html'), pinned: true });
    }
  });
});
